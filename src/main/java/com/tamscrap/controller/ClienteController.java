...
public class ClienteController {
	...
	public ResponseEntity<ClienteDTO> editarCliente(@PathVariable Long id, @RequestBody ClienteDTO clienteDTO) {
		...
		List<UserAuthority> authorities = clienteDTO.getAuthorities().stream().map(UserAuthority::valueOf)
				.collect(Collectors.toList());
	...
	private Cliente convertirACliente(ClienteDTO dto) {
		...
		List<UserAuthority> authorities = dto.getAuthorities().stream().map(UserAuthority::valueOf)
				.collect(Collectors.toList());
